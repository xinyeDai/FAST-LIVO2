...
//                                           (uint16_t, ring, ring)(double, timestamp, timestamp))POINT_CLOUD_REGISTER_POINT_STRUCT(HesaiPointXYZIRT,
POINT_CLOUD_REGISTER_POINT_STRUCT(HesaiPointXYZIRT,
...
void hesaiHandler_XYZI(sensor_msgs::PointCloud2 pc_msg)
{
              ...
              const typename pcl::PointCloud<T_out_p>::Ptr &pc_out)
{
    ...
    for (int point_id = 0; point_id < pc_in->points.size(); ++point_id)
    {
        ...
        // uint16_t ringid = pc_out->points[valid_point_id++].ring;
        // ROS_ERROR_STREAM("ringid:"<<ringid);
    }
    // ROS_ERROR_STREAM("count1:" << valid_point_id);
}
              ...
              const typename pcl::PointCloud<T_out_p>::Ptr &pc_out)
{
    ...
    // ROS_ERROR_STREAM("count2:" << valid_point_id);
}

void hesaiHandler_XYZIRT(const sensor_msgs::PointCloud2 &pc_msg)
{
    ...
    if (output_type == "XYZIRT")
    {
        ...
        handle_pc_msg<HesaiPointXYZIRT, VelodynePointXYZIRT>(pc_in, pc_out);
        add_ring<HesaiPointXYZIRT, VelodynePointXYZIRT>(pc_in, pc_out);
        add_time<HesaiPointXYZIRT, VelodynePointXYZIRT>(pc_in, pc_out);
    ...
    else if (output_type == "XYZIR")
    {
        ...
        handle_pc_msg<HesaiPointXYZIRT, VelodynePointXYZIR>(pc_in, pc_out);
        add_ring<HesaiPointXYZIRT, VelodynePointXYZIR>(pc_in, pc_out);
        publish_points(pc_out, pc_msg);
    }
    else if (output_type == "XYZI")
    {
        pcl::PointCloud<pcl::PointXYZI>::Ptr pc_out(new pcl::PointCloud<pcl::PointXYZI>());
        handle_pc_msg<HesaiPointXYZIRT, pcl::PointXYZI>(pc_in, pc_out);
        publish_points(pc_out, pc_msg);
    }
}
...
{
    ros::init(argc, argv, "hesai_converter");

    ros::console::set_logger_level(ROSCONSOLE_DEFAULT_NAME, ros::console::levels::Debug);
    ROS_INFO("****************");
    ros::NodeHandle nh;
    std::string lidar_topic;
    ros::param::get("common/hesai_topic", lidar_topic); // Retrieve the topic name parameter
    if (argc < 3)
    {
        ROS_ERROR(
    ...
    else
    {
        ...
        {
            subRobosensePC = nh.subscribe(lidar_topic, 1, hesaiHandler_XYZI);
        }
        else if (std::strcmp("XYZIRT", argv[1]) == 0)
        {
            subRobosensePC = nh.subscribe(lidar_topic, 1, hesaiHandler_XYZIRT);
        }
        else
        {
            ROS_ERROR(argv[1]);
    ...
    std::string publish_lidar_topic;
    ros::param::get("common/lid_topic", publish_lidar_topic); // Retrieve the topic name parameter

    pubRobosensePC = nh.advertise<sensor_msgs::PointCloud2>(publish_lidar_topic, 1);

    ROS_INFO("Listening to /hesailidar_points ......");
    ros::spin();
    return 0;
}
